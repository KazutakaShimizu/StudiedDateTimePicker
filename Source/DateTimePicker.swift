...
//    DateTimePicerViewを生成するメソッド
//    selectedはpickerを起動した際の初期値、minimuDateはどれだけ前の日にちまでさかのぼってpickerで選べるようにするか、maximumはどれだけ先の日付を選べるようにするか
    @objc open class func show(selected: Date? = nil, minimumDate: Date? = nil, maximumDate: Date? = nil) -> DateTimePicker {
        let dateTimePicker = DateTimePicker()
//        selectedがなければ、今日の日付を入れる
...
//        maximumDateとminimuDateを比較している
//        assert関数は、第一引数の条件がtrueであれば、第二引数の処理を実行する
...
//        このメソッドでviewを生成したり、位置などを設定しているはず
        dateTimePicker.configureView()
//        shared.keywindowで最前列のviewを取得して、そこにaddSubViewしてる
...
//        contentViewが存在するかをチェック
        if self.contentView != nil {
            self.contentView.removeFromSuperview()
        }
        let screenSize = UIScreen.main.bounds.size
        
//        dateTimePicker自体は、画面全体と同じサイズになっている
        self.frame = CGRect(x: 0,
                            y: 0,
                            width: screenSize.width,
                            height: screenSize.height)
        
//        pickerの部分はcontentView
        contentView = UIView(frame: CGRect(x: 0,
                                           y: frame.height,
                                           width: frame.width,
                                           height: contentHeight))
        
//        contentViewやcontentViewの影の位置や形を調整。hidden状態でaddSubviewする
...
//        Todayボタン
        ...
        todayButton.setTitle(todayButtonTitle, for: .normal)
        todayButton.setTitleColor(highlightColor, for: .normal)
//        クリックされた際、setTodayメソッドを呼び出す(追加の必要あり)
...
//        contentViewの下端から、height分+10のマージンをあけてる
        ...
        // hour table view
//        ドラムロールの時間の部分。これtableViewになってるのか・・・
...
//        時間同士を区切っている横棒
        ...
        // fill date
//        ドラムロールなどに入れる時間を作成
//        選択できる最大の過去の日付と、未来の日付をメソッドにわたしている
...
//        contentViewをアニメーションで下から出す
//        hiddenをいじるのではなく、positionを変更してアニメーションを作っている
...
//    fromDateにはカレンダーに表示する一番過去の日付が、toDateには未来の日付が入る
//    めんどくさそうなので後回し
